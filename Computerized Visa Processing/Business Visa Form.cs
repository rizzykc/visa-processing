...
{
    public partial class Business_Visa_Form : Form
    {
        string constStr, sql;
        MySqlCommand olecom;

        public Business_Visa_Form()
        {
            InitializeComponent();
        }
        ...
        {
            sql = "INSERT into business VALUES(S_no,?ePptNo,?First_Name,?Middle_Name,?Surname,?Sex,?D.O.B,?State_of_Origin,?Place_of_Birth,?Nationality,?Marital_Status,?Duration_of_Stay,?Mode_of_Transport,?Validity,?Next_of_Kin,?Next_of_Kin_No,?Signature)";
       ...
       private void mainData()
        {
            ...
            if (textBox8.Text != ""&& textBox8.Text=="0,9")
            {
                if (textBox6.Text != "")
                {
                    if (textBox5.Text != "")
        ...
        private void Business_Visa_Form_Load(object sender, EventArgs e)
        {
